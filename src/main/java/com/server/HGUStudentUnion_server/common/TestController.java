...
//        appUserService.save(AppUser.builder().build());
//
...
////        siteService.save(Site.builder().name("한동 팀모임 공간 대여 시스템").description("한동대학교 학생을 위한 팀모임 공간 대여 시스템입니다.").logo("null").link("mt2").company("(주)홍방이").maxDate(20).maxTime(120).restriction(3).question1("팀교수님").question2("팀장이름").timeUnit(30).build());
////    }
////
////    private void saveRoom() {
////        roomService.save(Room.builder().image(null).name("NTH 219").capacity(40).description("빔프로젝트 있음. 전자 칠판 있음. 깨끗하게 사용해주셔야 이후에 예약 가능합니다.").available(true).startTime(540).endTime(1260).reserveCnt(10).site(siteService.findById(1L)).build());
...
////        savedUserInfoService.save(SavedUserInfo.builder().groupName("조성배교수님팀").purpose("팀모임").reservation("배주영").contact("01011453212").authority(1).status(1).site(siteService.findById(2L)).user(userService.findByEmail("bae@handong.ac.kr")).build());
////        savedUserInfoService.save(SavedUserInfo.builder().groupName("남재창교수님팀").purpose("팀모임").reservation("정석민").contact("01011453212").authority(1).status(1).site(siteService.findById(3L)).user(userService.findByEmail("jung@handong.ac.kr")).build());
////
////    }
////
